...
export const colorSchemeScript = (forceColorScheme, defaultColorScheme) => {
  ...
  console.log(forceColorScheme, defaultColorScheme);
  if (forceColorScheme) {
  ...
  const colorSchemeValue = (storedColorScheme === "system" ? systemPreference : storedColorScheme) ?? defaultColorScheme ?? "system";
