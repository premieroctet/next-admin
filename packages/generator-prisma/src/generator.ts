...
const buildGeneratedFilesTo = async (filesToBuild: string[], outDir: string) => {
  await Promise.all((['cjs', 'esm'] as esbuild.Format[]).map(async (format) => {
...
export const updateNextAdminPrismaTypesImport = async (
  ...
  if (!fs.existsSync(path.resolve(generatorOutput))) {
    throw new Error(`Generator output path ${generatorOutput} does not exist, make sure to declare your prisma client BEFORE Next-Admin's one`);
  }

  const generatedPrismaInNextAdmin = path.join(nextAdminPath, 'generated', 'prisma');

  const generatedFiles = glob.sync("**/*.{js,mjs,ts,mts}", { cwd: generatorOutput, absolute: true })

  await buildGeneratedFilesTo(
    generatedFiles,
    generatedPrismaInNextAdmin
  )

  await buildGeneratedDtsFiles(
    generatedFiles,
    generatedPrismaInNextAdmin,
  )
  ...
  glob.sync("**/*.{js,mjs,ts}", { cwd: nextAdminPath }).forEach((file) => {
    ...
    let pathReplacement = path.relative(path.dirname(filePath), generatedPrismaInNextAdmin);
