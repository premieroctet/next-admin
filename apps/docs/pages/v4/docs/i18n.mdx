...
<Callout type="info">
  Using `getMessages` allows you to provide an object with a multiple level structure to translate the keys, while the `translations` props only allows you to provide a flat object (`form.widgets.file_upload.delete` ex.)
</Callout>
...
{/* > Note that if you only use one language for your admin, you should prefer to use the `alias` system ([more details](/v4/docs/api-docs#model)) to customize field names. */}
<Callout type="info">
  If you only use one language for your admin, you should prefer to use the `alias` system ([more details](/v4/docs/api-docs#model)) to customize field names.
</Callout>