...
By using [exports](/v4/docs/api/model-configuration#listexports-property) options, you can export data. Next Admin only implements the CSV export button, it's actually a link pointing to a provided url.
...
```typescript copy filename="app/api/users/export/route.ts"
...
```typescript copy filename="actions/nextadmin.ts"
"use server";
...
```typescript copy filename="/components/inputs/BirthDateInput.tsx"
"use client";
