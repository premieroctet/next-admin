...
<Steps>
  ...
  The `options` property is now fully optional, so we moved the `basePath` to the `<NextAdmin />` component. You can remove it from the options object.

  ### Remove server actions
  If you were using App Router you had to create server actions to perform some operations.
  In v5 these actions are removed and replaced by the `[[...nextadmin]]` route you just created. So you can remove them from your project.
