...
`model` is an object that represents the customization options for each model in your schema, it takes as **_key_** a model name of your schema as **_value_** an object to customize your model GUI (Graphical User Interface) with the following properties:
